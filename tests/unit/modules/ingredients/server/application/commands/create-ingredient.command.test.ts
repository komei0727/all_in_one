...
describe('CreateIngredientCommand', () => {
  describe('constructor', () => {
    it('すべての必須項目とオプション項目を含むコマンドを作成できる', () => {
      ...
      expect(command.expiryInfo).toBeTruthy()
      expect(command.expiryInfo?.bestBeforeDate).toBeTruthy()
      expect(command.expiryInfo?.useByDate).toBeTruthy()
    ...
    it('必須項目のみでコマンドを作成できる', () => {
      ...
      expect(command.expiryInfo).toBeUndefined()
      expect(command.purchaseDate).toBeTruthy()
