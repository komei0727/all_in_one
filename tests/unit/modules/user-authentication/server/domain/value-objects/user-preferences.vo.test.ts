...
describe('UserPreferences値オブジェクト', () => {
  ...
  describe('不正な値での作成', () => {
    it('無効なテーマで作成するとエラーが発生する', () => {
      ...
      // Act & Assert（実行 & 検証）
      expect(() => new UserPreferences(invalidPreferences))
        .toThrow('無効なテーマが指定されています')
    })
    ...
    it('nullで作成するとエラーが発生する', () => {
      ...
      // Act & Assert（実行 & 検証）
      expect(() => new UserPreferences(nullPreferences))
        .toThrow('ユーザー設定は必須です')
    })
    ...
    it('通知設定がbooleanでない場合エラーが発生する', () => {
      ...
      // Act & Assert（実行 & 検証）
      expect(() => new UserPreferences(invalidPreferences))
        .toThrow('通知設定はtrue/falseで指定してください')
    })
  })
