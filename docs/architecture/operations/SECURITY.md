  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      if (requiredRoles.length > 0) {
        const hasRequiredRole = requiredRoles.some((role) => context.user!.roles.includes(role))
...
const noControlChars = z
  .string()
  .refine((val) => !/[\x00-\x1f]/.test(val), { message: '制御文字は使用できません' })

const positiveNumber = z.number().refine((val) => val > 0, { message: '正の数を入力してください' })
